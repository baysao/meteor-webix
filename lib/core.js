...
function copy(obj, target, config) {
    ...
    for (var method in obj) {
        if (obj[method] && typeof obj[method] == "object" && !webix.isDate(obj[method]) && !obj.$proxy) {
...
function render_stack(layout, stack) {
    ...
    if (line) {
        ...
        WebixViews[url] = ui;
        //var ui = (typeof view == 'function') ? view() : view;
...
var app = {
    ...
    router: function (path) {
        if (document) {
            app.config.container = app.config.container || document.body;
            app.$layout.root = app.$layout.root || document.body;
        }
        //var path = app.config.getPath();
        var parts = parse_parts(path);
    ...
    show: function (name, options) {
        //app.config.goPath(name);
        app.config.router.go(name);
    },
    //start: function (name) {
    //    console.log('start');
    //    //init routing
...
function invalid_url(url) {
    ...
    if (app.debug)
        webix.message({type: "error", expire: 5000, text: "Can't load " + url});
